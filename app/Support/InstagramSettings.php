...
class InstagramSettings
{
    ...
    {
        return (bool) static::store($userId)->get('instagram_auto_queue_enabled', false);
    }

    public static function autoQueueIntervalMinutes(?int $userId = null): int
    {
        return max(1, min(1440, (int) static::store($userId)->get('instagram_auto_queue_interval_minutes', 60)));
    }
    ...
    {
        $state = static::store($userId)->get('instagram_auto_queue_state', []);
