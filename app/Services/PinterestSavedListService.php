...
class PinterestSavedListService
{
     ...
     */
    public function save(string $name, array $records, ?User $user = null, ?int $existingId = null): ?PinterestSavedList
    {
        $this->lastError = null;
        $name = trim($name);
        ...
        if ($existingId) {
            $list = PinterestSavedList::query()->find($existingId);
        ...
        return PinterestSavedList::query()->create([
            'user_id' => $user?->id,
            'name' => $name,
            'records' => $normalized->values()->all(),
            'record_count' => $normalized->count(),
    ...
    public function delete(int $id): bool
    {
        ...
        $deleted = PinterestSavedList::query()->whereKey($id)->delete();
    ...
    {
        $list = PinterestSavedList::query()->find($id);
