...
class InstagramSavedListService
{
     ...
     */
    public function save(string $name, array $records, ?User $user = null, ?int $existingId = null): ?InstagramSavedList
    {
        $this->lastError = null;
        $name = trim($name);
        ...
        if ($existingId) {
            $list = InstagramSavedList::query()->find($existingId);
        ...
        return InstagramSavedList::query()->create([
            'user_id' => $user?->id,
            'name' => $name,
            'records' => $normalized->values()->all(),
            'record_count' => $normalized->count(),
    ...
    public function delete(int $id): bool
    {
        ...
        $deleted = InstagramSavedList::query()->whereKey($id)->delete();
    ...
    {
        $list = InstagramSavedList::query()->find($id);
